##main.py is the application entry point that starts the entire multi-agent customer support system.
#  It first downloads and prepares the SQLite database (if needed), generates a visual diagram of the LangGraph workflow,
#  creates a unique conversation thread, and configures the session with the current user's passenger_id. It then runs an interactive chat
# loop where every user query is sent to the compiled LangGraph, streams and displays the assistant's responses, and manages human-in-the-loop
# approval for sensitive operations such as bookings, updates, or cancellations. If the graph pauses before executing
#  a sensitive tool, main.py asks the user for approval (y) or feedback, resumes or modifies the graph execution accordingly,
# and continues the conversation. In simple terms, main.py initializes the system, runs the chatbot, and coordinates user interaction with the multi-agent graph.
import uuid
import os  # Import os module for file operations
from agent.app.graph import multi_agentic_graph
from agent.app.services.utils import download_and_prepare_db
